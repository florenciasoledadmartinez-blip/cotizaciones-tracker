...
// Tasks that only leader/admin can tick
const LEADER_ONLY_TASKS = new Set(['Validación interna']);
...
export default function ChecklistPanel({
  quoteId, tasks, quoteStatus, canEdit, onRefresh, currentUserName, currentUserRole,
}: ChecklistPanelProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {localTasks.map(task => {
        ...
        // This task is restricted to leader/admin — operator can view but not modify
        const isLeaderOnly = LEADER_ONLY_TASKS.has(task.template_name);
        const taskCanEdit = canEdit && !(isLeaderOnly && currentUserRole === 'operator');
              ...
              <div className="border-t border-gray-100">
                {task.subtasks.map(sub => (
                  <div key={sub.id}
                    ...
                    ) : isLeaderOnly && canEdit ? (
                      /* Operator viewing a leader-only task: show lock */
                      <div className="w-5 h-5 rounded border-2 border-purple-200 bg-purple-50 flex-shrink-0 mt-0.5 flex items-center justify-center"
                        title="Solo el líder puede modificar esta etapa">
